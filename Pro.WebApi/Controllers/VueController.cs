...
namespace Pro.WebApi.Controllers
{
    ...
    { /// <summary>
      ...
      /// <returns></returns>
        [HttpPost]
        public AjaxMessage UploadFileData(string data, string baseArr)
        {
            var ajax = new AjaxMessage();
            ajax.IsSuccess = false;
            ajax.Message = "系统异常,请联系管理员";
            //var json = new XJsonResult(new { success = false, });
            try
            {
                ...
                if (m_formData != null)
                {
                    ...
                    if (m_fileData != null)
                    {
                        var path = m_fileData[0].url;
                        Image img = ImgHelper.ReadImageFile(path);

                        if (img != null)
                        {
                            string imageName = DateTime.Now.ToString("yyyyMMDDHHmmss") + ".jpg";
                            string baseContent = ImgHelper.ImageToBase64(img);
                            var byteData = ImgHelper.ToBase64Bytes(baseContent);
                            ImgHelper.SaveImage(byteData, imageName, "测试", ref path);
                        }
        ...
        /// <returns></returns>
        [HttpPost]
        public AjaxMessage UploadImg(HttpPostedFileBase fileData, string path)
        {
            var ajax = new AjaxMessage();
            ajax.IsSuccess = false;
            ajax.Message = "系统异常,请联系管理员";
        ...
        {
            var currentPth = System.Environment.CurrentDirectory;
            string phyPath = Directory.GetCurrentDirectory() + filepath;
